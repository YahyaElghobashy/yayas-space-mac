...
summary](https://github.com/YahyaElghobashy/yayas-space-mac/issues/838) sorts
...
Yaya's Space is one menu bar icon doing the job of a dozen paid Mac apps. Doing
...
is that every upstream breakage arrives here as a Yaya's Space bug, reported by
...
- **`./build.sh --test` is not proof that it compiles.** It builds a
  hand-written list of source files, and `Sources/YayasSpace/UI/` is largely
