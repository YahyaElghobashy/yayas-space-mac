...
sleep() {
    if (( remaining_writes > 0 )); then
        print '{}' > "$cleanup_test_dir/com.yahyaelghobashy.yayasspace.tests.delayed.plist"
        remaining_writes=$((remaining_writes - 1))
    fi
}
print '{}' > "$cleanup_test_dir/yayasspace.tests.first.plist"
...
[[ ! -e "$cleanup_test_dir/com.yahyaelghobashy.yayasspace.tests.delayed.plist" ]]
[[ ! -e "$cleanup_test_dir/yayasspace.tests.first.plist" ]]
