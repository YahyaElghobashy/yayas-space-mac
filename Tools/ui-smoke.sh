...
APP="/Applications/Yaya's Space (Developer).app"
PROCESS="YayasSpaceDeveloper"
OUT="${1:-$(mktemp -d /tmp/yayasspace-ui-smoke.XXXXXX)}"
mkdir -p "$OUT"
FAILURES=0
...
sleep 1.5
QP=$(ax "get position of window \"Yaya's Space\"")
if [[ -n "${QP:-}" ]]; then
...
sleep 1.5
SW=$(ax "get position of window \"Yaya's Space Settings\"")
if [[ -n "${SW:-}" ]]; then
    pass "settings window at $SW"
    screencapture -x "$OUT/settings.png"
    ax "click button 1 of window \"Yaya's Space Settings\"" >/dev/null
else
