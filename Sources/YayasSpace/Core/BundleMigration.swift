...
/// OLD PATH ("/Applications/Yaya's Space Utils.app"), so right after updating the
/// app is running from a bundle still named "Yaya's Space Utils.app". We rename
/// that bundle to "Yaya's Space.app" through a detached helper that runs only after
...
///
/// When we are already running as "Yaya's Space.app", we instead retire a stray
...
enum BundleMigration {
    private static let oldName = "Yaya's Space Utils.app"
    private static let newName = "Yaya's Space.app"
    ...
    static func run() -> Bool {
        // Yaya's Space never shipped under another bundle name, so there is
        // nothing to migrate and no stray bundle to retire. The helpers below
        // stay for reference but are never reached.
        return false
    }

    private static func runUpstreamMigration() -> Bool {
    ...
    private static func renameSelfAndRelaunch(from old: URL) -> Bool {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("yayasspace-rename-\(pid)-\(UUID().uuidString).sh")
        do {
