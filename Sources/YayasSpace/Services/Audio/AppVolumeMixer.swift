...
final class AppVolumeMixer: ObservableObject {
    ...
    private let buildQueue = DispatchQueue(label: "com.yahyaelghobashy.yayasspace.mixer", qos: .userInitiated)
    ...
    private let halQueue = DispatchQueue(label: "com.yahyaelghobashy.yayasspace.mixer.hal", qos: .userInitiated)
    ...
    private static func outputDevices(defaultUID: String?) -> [MixerOutputDevice] {
        ...
        for deviceID in deviceIDs {
            ...
            let name = read(deviceID, kAudioObjectPropertyName, &nameRef)
                ? nameRef as String
                : uid
            guard name != "Yaya's Space Mixer" else { continue }
...
private final class TapGainEngine: GainEngine {
    ...
    init?(objects: [AudioObjectID], gain: Float, outputDeviceUID: String) {
        ...
        let aggregate: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Yaya's Space Mixer",
    ...
    private static let rateQueue = DispatchQueue(label: "com.yahyaelghobashy.yayasspace.mixer.rate",
                                                 qos: .userInitiated)
    ...
    private static let teardownQueue: OperationQueue = {
        ...
        queue.name = "com.yahyaelghobashy.yayasspace.mixer.teardown"
