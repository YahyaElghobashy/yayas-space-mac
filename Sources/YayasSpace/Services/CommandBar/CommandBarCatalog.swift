...
enum CommandBarCatalog {
    ...
    /// showing its own shortcut. This is how the bar reaches past Yaya's Space
    ...
    /// runs, so one row serves both "gh" and "gh yayasspace".
    ...
    private static func keepOnShelf(_ text: String) {
        let board = NSPasteboard(name: NSPasteboard.Name("com.yahyaelghobashy.yayasspace.commandbar.selection"))
        board.clearContents()
