...
final class RecorderCaptureEngine: NSObject {
    ...
    private let queue = DispatchQueue(label: "com.yahyaelghobashy.yayasspace.recorder.capture",
                                      qos: .userInitiated)
    ...
    /// Builds the filter and starts the stream. Existing ordinary Yaya's Space
...
final class RecorderMicrophoneCapture: NSObject,
    ...
    private let queue = DispatchQueue(label: "com.yahyaelghobashy.yayasspace.recorder.microphone",
                                      qos: .userInitiated)
