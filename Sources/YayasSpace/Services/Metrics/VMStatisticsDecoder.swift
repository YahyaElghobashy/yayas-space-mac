...
enum VMStatisticsDecoder {
    static let rev1Count = mach_msg_type_number_t(YAYASSPACE_HOST_VM_INFO64_REV1_COUNT)
    static let rev2Count = mach_msg_type_number_t(YAYASSPACE_HOST_VM_INFO64_REV2_COUNT)
    static let rev3Count = mach_msg_type_number_t(YAYASSPACE_HOST_VM_INFO64_REV3_COUNT)

    static func read() -> VMStatisticsSnapshot? {
        var raw = yayasspace_vm_statistics64_rev3_t()
        var returnedCount = mach_msg_type_number_t()
        guard yayasspace_read_vm_statistics64(&raw, &returnedCount) == KERN_SUCCESS else { return nil }
        return decode(raw, returnedCount: returnedCount)
    }

    static func decode(_ raw: yayasspace_vm_statistics64_rev3_t,
