...
enum AdminShell {
    // Yaya's Space is a menu-bar agent (LSUIElement), so it is rarely the active
...
enum Sudoers {
    static let rulePath = "/etc/sudoers.d/yayasspace-clamshell"
    // Rule files written under earlier names. Yaya's Space never shipped under
    // another name, so this list is empty; it deliberately does not touch the
    // upstream app's own rule file.
    ...
    private static let sleepStateQueue = DispatchQueue(label: "com.yahyaelghobashy.yayasspace.pmset-state")
