...
struct EnergySettings: View {
    ...
    @AppStorage(DefaultsKey.keepAwakeActiveIcon) private var keepAwakeActiveIcon = KeepAwakeActiveIcon.yayasspace.rawValue
    ...
    private var aboutContent: some View {
        VStack(spacing: 14) {
            ...
            Text(AppInfo.copyright)
                .font(.caption2)
                .foregroundStyle(.tertiary)
            // One-line upstream attribution (GPL-3.0-or-later fork of Vorssaint).
            HStack(spacing: 4) {
                Text(AppInfo.attributionShort)
                Link(AppInfo.upstreamName, destination: AppInfo.upstreamRepositoryURL)
            }
            .font(.caption2)
            .foregroundStyle(.tertiary)
        }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                ...
                // Yaya's Space: no donation, chat or social links. The fork is
                // a personal project; the only calls to action are the repo
                // and the upstream project it is based on.
                VStack(spacing: 7) {
                    Text(AppInfo.supportHeading)
                        .font(.title2.bold())
                        .multilineTextAlignment(.center)
                ...
                HStack(alignment: .top, spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 7) {
                        Text(AppInfo.upstreamHeading)
                            .font(.headline)
                        Text(AppInfo.upstreamMessage)
                            .font(.system(size: 13))
                            .foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)

                        Button {
                            openURL(AppInfo.upstreamRepositoryURL)
                        } label: {
                            Label(AppInfo.upstreamButtonTitle, systemImage: "arrow.up.right.square")
                        }
                        .buttonStyle(.bordered)
                        .controlSize(.large)
                        .padding(.top, 3)
