...
enum Theme {
    ...
    /// Yaya's Space brand colours (same values as the app icon).
    static let brandPaper = Color(red: 0xF8 / 255, green: 0xF5 / 255, blue: 0xF0 / 255)
    static let brandPlum = Color(red: 0x46 / 255, green: 0x21 / 255, blue: 0x6B / 255)
...
/// Squircle badge with the mark on paper — the app's face in the About tab
/// and onboarding, matching the app icon (paper #F8F5F0, plum #46216B).
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: size * 0.26, style: .continuous)
                .fill(Theme.brandPaper)
                .overlay(
                    RoundedRectangle(cornerRadius: size * 0.26, style: .continuous)
                        .strokeBorder(Theme.brandPlum.opacity(0.18), lineWidth: 1)
                )
            BrandMark(width: size * 0.62, tint: Theme.brandPlum)
        }
